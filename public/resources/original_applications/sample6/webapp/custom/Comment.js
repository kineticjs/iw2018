...
sap.ui.define(['jquery.sap.global', 'sap/m/library', 'sap/ui/core/Control'],
	function (jQuery, library, Control) {
		...
		var Comment = Control.extend("custom.Comment", {
			metadata: {
				properties: {
					/**
					 * Text in HTML format.
		...
		/*
		 * these are the rules for the Comment
		 */
		var _renderingRules = {};
		...
		var SCHEMA_NAME_PATTERN = /^[a-z]+:*/i;

		/**
		 * Initialization hook for the Comment, which creates a list of rules with allowed tags and attributes.
		 */
		...
		function fnSanitizeAttribs (tagName, attribs) {
			...
			for (var i = 0; i < attribs.length; i += 2) {
				...
				if (!_renderingRules.ATTRIBS[attr] && !_renderingRules.ATTRIBS[tagName + "::" + attr]) {
					sWarning = 'Comment: <' + tagName + '> with attribute [' + attr + '="' + value + '"] is not allowed';
				...
				// sanitize hrefs
				if (attr == "href") { // a::href
					var sUrl = value.replace(SCHEMA_NAME_PATTERN, "");
		...
		Comment.prototype.onAfterRendering = function () {
		 ...
		 * @return {Comment} this for chaining
		 * @public
		 */
		Comment.prototype.setHtmlText = function (sText) {
			var sSanitizedText = "";
